...
const FIBER_METHODS = new Map<string, EndpointNode['metadata']['method']>([
  ['Get', 'GET'], ['Post', 'POST'], ['Put', 'PUT'], ['Patch', 'PATCH'],
  ['Delete', 'DELETE'], ['Head', 'HEAD'], ['Options', 'OPTIONS'],
  ['All', 'GET'],
]);

export function extractFiberEndpoints(
  rootNode: SyntaxNode,
  filePath: string,
): EndpointNode[] {
  ...
  for (const call of findAll(rootNode, 'call_expression')) {
    ...
    const httpMethod = FIBER_METHODS.get(method);
    if (!httpMethod) continue;
    if (!/^app\b|fiber|router|api|v\d/i.test(obj)) continue;
