...
export function extractTelemetry(
  ...
  for (const call of calls) {
    ...
    if (fn.type === 'member_expression') {
      ...
      const spanName = firstArg ? (extractStringValue(firstArg) ?? firstArg.text.slice(0, 50)) : method;

      // Span operations
      if (method === 'startSpan' || method === 'startActiveSpan' || method === 'startSegment') {
        const id = nodeId('telemetry', filePath, loc.line, `span:${spanName}`);
        nodes.push({
          id,
          type: 'telemetry',
          name: spanName,
      ...
      if (/createCounter|createHistogram|createGauge|createUpDownCounter/.test(method)) {
        const metricType = method.replace('create', '').toLowerCase() as TelemetryNode['metadata']['metric'] extends { type: infer T } ? T : never;
        const id = nodeId('telemetry', filePath, loc.line, `metric:${spanName}`);
        nodes.push({
          id,
          type: 'telemetry',
          name: spanName,
      ...
      if (method === 'propagate' || method === 'inject' || method === 'extract') {
        const id = nodeId('telemetry', filePath, loc.line, `context:${method}`);
        nodes.push({
          id,
          type: 'telemetry',
          name: `context.${method}`,
  ...
  for (const cls of classes) {
    const methods = findAll(cls, 'method_definition');
    for (const method of methods) {
      const decs = getDecorators(method);
      for (const dec of decs) {
        const name = decoratorName(dec);
        if (/span|trace|instrument/i.test(name)) {
          const loc = toLocation(method, filePath);
          const id = nodeId('telemetry', filePath, loc.line, `decorator:${name}`);
...
function detectSDK(objPath: string): SDK | null {
  if (OTEL_TRACER_PATTERNS.some(p => p.test(objPath))) return 'otel';
  if (OTEL_METER_PATTERNS.some(p => p.test(objPath))) return 'otel';
  if (DATADOG_PATTERNS.some(p => p.test(objPath))) return 'datadog';
  if (NEWRELIC_PATTERNS.some(p => p.test(objPath))) return 'newrelic';
  return null;
}
...
function detectSpanKind(spanName: string): 'internal' | 'server' | 'client' | 'producer' | 'consumer' {
  ...
  if (/produce|publish|send|emit/.test(lower)) return 'producer';
  if (/consume|subscribe|listen/.test(lower)) return 'consumer';
  return 'internal';
}
