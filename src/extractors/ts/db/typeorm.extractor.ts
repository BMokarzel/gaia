...
import {
  findAll, toLocation, memberChain, getDecorators, decoratorName,
  decoratorFirstArg, getDecoratorsByName, fieldText,
...
const TYPEORM_OPERATIONS: Record<string, DbProcessNode['metadata']['operation']> = {
  find: 'findMany',
  findOne: 'findFirst',
  findOneBy: 'findFirst',
 ...
 */
export function extractTypeORMOperations(
  rootNode: SyntaxNode,
  filePath: string,
): TypeORMExtractionResult {
...
function extractEntitySchema(
  ...
  for (const classNode of classes) {
    const entityDec = getDecoratorsByName(classNode, 'Entity');
    if (entityDec.length === 0) continue;

    // Nome da tabela: @Entity('users') ou derivado do nome da classe
    const entityArg = decoratorFirstArg(entityDec[0]);
    ...
    if (classBody) {
      const properties = findAll(classBody, 'public_field_definition');
...
function extractColumnDef(propNode: SyntaxNode): ColumnDef | null {
  const decorators = getDecorators(propNode);
  if (decorators.length === 0) return null;

  const hasColumn = decorators.some(d =>
    ['Column', 'PrimaryColumn', 'PrimaryGeneratedColumn', 'CreateDateColumn',
     'UpdateDateColumn', 'DeleteDateColumn', 'VersionColumn', 'Generated'].includes(decoratorName(d))
  );

  if (!hasColumn) return null;

  const propName = propNode.childForFieldName('name')?.text ?? '';
  const decoratorNames = decorators.map(d => decoratorName(d));

  const isPrimary = decoratorNames.some(n =>
    n === 'PrimaryGeneratedColumn' || n === 'PrimaryColumn'
  );
  ...
  const columnDec = decorators.find(d => decoratorName(d) === 'Column');
  let colType = 'varchar';
  ...
  if (columnDec) {
    ...
    if (args) {
      ...
      if (text.includes('nullable: false')) nullable = false;
      if (text.includes('unique: true')) unique = true;
    }
  }

  if (isCreateDate || isUpdateDate) colType = 'timestamp';
  if (isDeleteDate) { colType = 'timestamp'; nullable = true; }
...
function inferModelFromRepoName(repoPath: string): string {
  // this.userRepository → 'user'
  // this.userRepo → 'user'
  // userRepository → 'user'
