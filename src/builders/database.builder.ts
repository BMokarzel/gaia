import type { DatabaseNode, TableNode } from '../types/topology';
 ...
 */
export function mergeDatabases(databases: DatabaseNode[]): DatabaseNode[] {
  const byAlias = new Map<string, DatabaseNode>();
...
function mergeTables(target: TableNode, source: TableNode): void {
  if (!target.metadata.columns) target.metadata.columns = [];
  ...
  if (source.metadata.hasTimestamps) target.metadata.hasTimestamps = true;
  if (source.metadata.hasSoftDelete) target.metadata.hasSoftDelete = true;
 ...
 */
export function buildDatabaseFromHint(
  alias: string,
  engine: string,
  orm?: string,
): DatabaseNode {
  return {
    id: resourceId('database', alias),
    type: 'database',
    name: alias,
    metadata: {
      engine: engine as DatabaseNode['metadata']['engine'],
...
function engineToCategory(engine: string): DatabaseNode['metadata']['category'] {
  ...
  if (sql.includes(engine)) return 'sql';
  if (nosql.includes(engine)) return 'nosql';
  if (kv.includes(engine)) return 'kv';
  if (search.includes(engine)) return 'search';
  if (analytics.includes(engine)) return 'analytics';
  if (graph.includes(engine)) return 'graph';
  if (timeseries.includes(engine)) return 'timeseries';
  return 'nosql';
}
