...
export async function runCrossServiceMerge(
  ...
  const serviceNames = new Map(services.map(s => [s.id, s.metadata.fullName || s.name]));
  ...
  for (const extCall of externalCalls) {
    ...
    if (candidates.length === 1 && candidates[0].confidence >= 0.95) {
      ...
      applyResolution(extCall, candidates[0].endpointId, candidates[0].confidence, 'unique high-confidence match', edges);
      continue;
    }
    ...
    try {
      ...
      if (result.certain && result.resolvedEndpointId !== 'unresolvable') {
        applyResolution(extCall, result.resolvedEndpointId, result.confidence, result.reason, edges);
      } else {
...
export function applyPendingMerges(
  ...
  for (const entry of file.pendingMerges) {
    ...
    applyResolution(extCall, entry.decision, 1.0, 'user decision', edges);
  }
...
function computeConfidence(extCall: ExternalCallNode, ep: EndpointEntry): number {
  ...
  const extParams = (extCall.metadata.pathNormalized.match(/:param/g) ?? []).length;
