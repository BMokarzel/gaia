...
exports.extractSpringEndpoints = extractSpringEndpoints;
const ast_helpers_1 = require("../../../utils/ast-helpers");
const id_1 = require("../../../utils/id");
/** Annotations Spring MVC → HTTP method */
const SPRING_HTTP_ANNOTATIONS = {
    GetMapping: 'GET',
    PostMapping: 'POST',
    PutMapping: 'PUT',
    PatchMapping: 'PATCH',
 ...
 */
function extractSpringEndpoints(rootNode, filePath) {
    const endpoints = [];
    const functions = [];
    ...
    for (const classNode of classes) {
        ...
        const classRequestMapping = annotations.find(a => a.childForFieldName('name')?.text === 'RequestMapping');
        const basePath = extractAnnotationValue(classRequestMapping) ?? '';
        const className = (0, ast_helpers_1.fieldText)(classNode, 'name') ?? 'UnknownController';
        // Encontra métodos da classe
        const classBody = classNode.childForFieldName('body') ?? classNode;
        const methods = (0, ast_helpers_1.findAll)(classBody, 'method_declaration');
        for (const method of methods) {
            const methodAnnotations = (0, ast_helpers_1.findAll)(method, 'marker_annotation')
                .concat((0, ast_helpers_1.findAll)(method, 'annotation'));
            ...
            if (annotationName === 'RequestMapping') {
                ...
                if (args) {
                    ...
                    if (methodAttr.includes('POST'))
                        httpMethod = 'POST';
                    else if (methodAttr.includes('PUT'))
                        httpMethod = 'PUT';
                    else if (methodAttr.includes('PATCH'))
                        httpMethod = 'PATCH';
                    else if (methodAttr.includes('DELETE'))
                        httpMethod = 'DELETE';
                }
            }
...
function extractSpringParams(method) {
    const params = [];
    const query = [];
    const body = [];
    const headers = [];
    ...
    for (const param of formalParams.namedChildren) {
        ...
        const annotations = (0, ast_helpers_1.findAll)(param, 'marker_annotation').concat((0, ast_helpers_1.findAll)(param, 'annotation'));
...
function buildJavaFunction(method, filePath, className) {
    const loc = (0, ast_helpers_1.toLocation)(method, filePath);
    const methodName = (0, ast_helpers_1.fieldText)(method, 'name') ?? 'unknown';
    const id = (0, id_1.nodeId)('function', filePath, loc.line, `${className}.${methodName}`);
    const modifiers = (0, ast_helpers_1.findAll)(method, 'modifiers');
 ...
 */
function extractAnnotationValue(annotation) {
    if (!annotation)
        return null;
