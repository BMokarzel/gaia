 ...
 */
export function extractFastAPIEndpoints(
  rootNode: SyntaxNode,
  filePath: string,
): EndpointNode[] {
  ...
  for (const def of decorated) {
    ...
    for (const decorator of decorators) {
      ...
      const path = firstArg ? extractStringValue(firstArg) : null;
      if (!path) continue;
...
function extractFastAPIParams(funcDef: SyntaxNode, path: string): EndpointNode['metadata']['request'] {
  ...
  if (parameters) {
    ...
    for (const param of parameters.namedChildren) {
      if (param.type !== 'identifier' && param.type !== 'typed_parameter' &&
          param.type !== 'typed_default_parameter') continue;

      const name = param.childForFieldName('name')?.text ?? param.text.split(':')[0].trim();
      if (!name || name === 'self' || name === 'request' || name === 'response') continue;
